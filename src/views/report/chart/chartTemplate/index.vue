<template>
  <div class="app-container">
    <el-form :model="queryParams" ref="queryRef" :inline="true">
      ...
      <el-form-item>
        <el-button type="primary" icon="search" @click="handleQuery" v-hasPermi="['report:chart:chartTemplate:list']">搜索</el-button>
    ...
    <el-row :gutter="10" class="mb8">
      <el-col :span="1.5">
        <el-button
          ...
          icon="plus"
          @click="handleCreate"
          v-hasPermi="['report:chart:chartTemplate:create']">新增</el-button>
      </el-col>
      <el-col :span="1.5">
        <el-button
          type="success"
          ...
          :disabled="single"
          @click="handleEdit"
          v-hasPermi="['report:chart:chartTemplate:edit']">修改</el-button>
      </el-col>
      <el-col :span="1.5">
        <el-button
          type="danger"
          ...
          :disabled="multiple"
          @click="handleDelete"
          v-hasPermi="['report:chart:chartTemplate:delete']">删除</el-button>
      </el-col>
    </el-row>
    <!--列表数据-->
    <el-table v-loading="loading" :data="chartTemplateList" @selection-change="handleSelectionChange">
      ...
      <el-table-column label="图表类型" align="center" min-width="120" :show-overflow-tooltip="true" >
        <template #default="scope">
          <span>{{ scope.row.chartTypeName }}</span>
        </template>
      </el-table-column>
      ...
      <el-table-column label="操作" align="center" width="140" fixed="right" class-name="small-padding fixed-width">
        <template #default="scope">
          <el-button
            ...
            v-hasPermi="['report:chart:chartTemplate:edit']">修改
          </el-button>
          <el-button
            link
            type="danger"
            ...
            v-hasPermi="['report:chart:chartTemplate:delete']">删除
          </el-button>
        </template>
      </el-table-column>
    </el-table>
    ...
    <!-- 表单 -->
    <ChartTemplateForm ref="formRef" @success="getList" />
...
<script setup name="ChartTemplate">
  import { fetchList, deleteChartTemplate } from "@/api/report/chart/chartTemplate";
  import ChartTemplateForm from './form.vue'
  ...
  function handleBindList(row){
    statBindConfigListRef.value.showData(row.templateId,'CHART')
  }
  
  /** 查询列表 */
  function getList() {
    loading.value = true;
    chartTemplateList.value = [];
    fetchList(queryParams.value).then(
      response => {
        chartTemplateList.value = response.rows;
  ...
  function handleDelete(row) {
    ...
    proxy.$confirm('是否确认删除编号为"' + deleteIds + '"的数据项?', "警告", {
      ...
      return deleteChartTemplate(deleteIds);
    }).then(() => {
