  ...
  <el-dialog :title="title" v-model="open" width="400px" append-to-body>
    <el-form ref="formRef" :model="form" :rules="rules" v-loading="formLoading" label-width="80px">
      <el-form-item label="名称" prop="instrumentName">
        <el-input v-model="form.instrumentName" placeholder="请输入名称" />
      </el-form-item>
...
<script setup name="InstrumentForm">
  import { createInstrument, editInstrument, getInstrument } from "@/api/music/instrument";
  ...
  function submitForm() {
    proxy.$refs["formRef"].validate(valid => {
      if (valid) {
        if (form.value.instrumentId != undefined) {
          ...
          createInstrument(form.value).then(response => {
