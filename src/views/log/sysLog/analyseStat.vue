<template>
  <div class="app-container">
    <el-form :model="queryParams" ref="queryRef" :inline="true">
      ...
      <el-form-item>
        <el-button type="primary" icon="TrendCharts" @click="handleQuery" v-hasPermi="['log:sysLog:analyseStat']">统计</el-button>
...
<script setup name="SysLogAnalyseStat">
  import { onMounted } from 'vue'
  import { getSysLogAnalyseStat } from "@/api/log/sysLog";
  ...
  function initChart() {
    ...
    getSysLogAnalyseStat(proxy.addDateRange(queryParams.value, dateRange.value)).then(
      response => {
  ...
  onMounted(() => {
    ...
    initChart();
    //功能类型
    proxy.getEnumDict('LogLevel', 'ORDINAL', false).then(response => {
      logLevelOptions.value = response;
    });
    proxy.getDictItemTree('SYSTEM_LOG_DATE_GROUP_FIELD', false).then(response => {
