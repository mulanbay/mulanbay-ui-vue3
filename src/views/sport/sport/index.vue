    ...
    <el-row :gutter="10" class="mb8">
      <el-col :span="1.5">
        <el-button
          ...
          icon="plus"
          @click="handleCreate"
          v-hasPermi="['sport:sport:create']">新增</el-button>
      </el-col>
      <el-col :span="1.5">
        <el-button
          type="success"
          ...
          :disabled="single"
          @click="handleEdit"
          v-hasPermi="['sport:sport:edit']">修改</el-button>
      </el-col>
      <el-col :span="1.5">
        <el-button
          type="danger"
          ...
          :disabled="multiple"
          @click="handleDelete"
          v-hasPermi="['sport:sport:delete']">删除</el-button>
      </el-col>
    </el-row>

    <!--列表数据-->
    <el-table v-loading="loading" :data="sportList" @selection-change="handleSelectionChange">
      <el-table-column type="selection" width="55" align="center" />
      <el-table-column label="ID" fixed="left" prop="sportId" sortable="custom" align="center" width="120">
      ...
      <el-table-column label="统计" width="50" :show-overflow-tooltip="true">
        <template #default="scope">
          <span class="link-type" @click="handleMultiStat(scope.row.sportId)"><el-icon>
              <Histogram />
            </el-icon></span>
        </template>
      </el-table-column>
      ...
      <el-table-column label="操作" align="center" width="210" fixed="right" class-name="small-padding fixed-width">
        <template #default="scope">
          <el-button
            ...
            v-hasPermi="['sport:sport:edit']">修改
          </el-button>
          <el-button
            link
            type="danger"
            ...
            v-hasPermi="['sport:sport:delete']">删除
          </el-button>
        </template>
      </el-table-column>
    </el-table>
...
<script setup name="Sport">
  ...
  function handleEdit(row) {
    const id = row.sportId || ids.value.join(",");
    formRef.value.openForm(id, 'edit');
  }

  /** 删除按钮操作 */
  function handleDelete(row) {
    const deleteIds = row.sportId || ids.value.join(",");
    proxy.$confirm('是否确认删除编号为"' + deleteIds + '"的数据项?', "警告", {
      confirmButtonText: "确定",
      cancelButtonText: "取消",
      type: "warning"
    }).then(function() {
      return deleteSport(deleteIds);
    }).then(() => {
  ...
  function handleSelectionChange(selection) {
    ids.value = selection.map(item => item.sportId)
